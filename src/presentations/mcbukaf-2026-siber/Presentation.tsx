    ...
    <div className="flex flex-col items-center justify-center h-full text-center px-8 relative overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col items-center">
        <motion.div initial={{ scale: 0, rotate: -180 }} animate={{ scale: 1, rotate: 0 }} transition={{ type: "spring", stiffness: 120, damping: 10 }}
          className="text-[6rem] mb-8" style={{ filter: `drop-shadow(0 0 20px ${color}50)` }}>{icon}</motion.div>
        <motion.h1 initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.6 }}
          className="mcb-h1 font-black tracking-tight mb-5" style={{ color, textShadow: `0 0 20px ${color}60, 0 0 60px ${color}25` }}>
      ...
      <div className="space-y-4 w-full max-w-6xl">
        {items.map((item, i) => (
          <motion.div key={i} initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.12 * (i + 1), type: "spring", stiffness: 80 }}
            className={`flex items-start gap-5 border-l-4 ${borderClr} bg-white/[0.03] backdrop-blur-sm rounded-r-xl pl-8 py-5`}>
            <span className="text-4xl shrink-0">{item.emoji}</span>
            <p className="mcb-body text-gray-200">{item.text}</p>
          </motion.div>
        ))}
      </div>
      {note && <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1 }} className="mt-8 mcb-meta text-gray-500 italic text-center max-w-4xl">{note}</motion.p>}
    </div>
  );
}
...
function AnimatedStat({ value, label, color, delay }: { value: string; label: string; color: string; delay: number }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 40, scale: 0.8 }} animate={{ opacity: 1, y: 0, scale: 1 }}
      ...
      <p className="mcb-stat font-black mcb-mono mb-4" style={{ color, textShadow: `0 0 18px ${color}55, 0 0 45px ${color}25` }}>{animated}</p>
      <p className="mcb-meta text-gray-400 max-w-[280px] mx-auto">{label}</p>
    </motion.div>
  );
}
...
function StatSlide({ title, stats }: { title: string; stats: { value: string; label: string; color: string }[] }) {
  return (
    <div className="flex flex-col items-center justify-center h-full px-10 sm:px-20">
      <motion.h2 initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mcb-h2 font-bold mb-20 text-center">{title}</motion.h2>
...
function QuoteSlide({ quote, author, emoji }: { quote: string; author: string; emoji: string }) {
  return (
    <div className="flex flex-col items-center justify-center h-full px-12 sm:px-24 text-center">
      ...
      <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1 }} className="mt-10 mcb-lead text-gray-500">— {author}</motion.p>
    </div>
  );
}
...
function BigTextSlide({ text, subtext, color = "#00ff88" }: { text: string; subtext?: string; color?: string }) {
  return (
    <div className="flex flex-col items-center justify-center h-full px-12 sm:px-24 text-center">
      ...
      {subtext && (
        <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}
          className="mt-10 mcb-lead text-gray-400 max-w-4xl">{subtext}</motion.p>
      )}
    </div>
  );
}
    ...
    <div className="flex flex-col items-center justify-center h-full px-10 sm:px-20">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 w-full max-w-6xl">
        <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}
          className="rounded-xl pl-8 py-6 bg-white/[0.03] backdrop-blur-sm border-l-4"
          style={{ borderColor: lc, boxShadow: `0 0 18px ${lc}25` }}>
          <h3 className="mcb-h3 font-bold mb-5" style={{ color: lc }}>{left.title}</h3>
          <ul className="space-y-4">
            {left.items.map((item, i) => (
              <li key={i} className="mcb-body text-gray-300 flex items-start gap-3">
                <span style={{ color: lc }} className="mt-0.5">✕</span>{item}
              </li>
            ))}
          </ul>
        </motion.div>
        <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 }}
          ...
          <h3 className="mcb-h3 font-bold mb-5" style={{ color: rc }}>{right.title}</h3>
          <ul className="space-y-4">
            {right.items.map((item, i) => (
              <li key={i} className="mcb-body text-gray-300 flex items-start gap-3">
                <span style={{ color: rc }} className="mt-0.5">✓</span>{item}
              </li>
            ))}
          </ul>
        </motion.div>
...
function ClickReveal({ title, icon, layers, ctx, accent = "#00ff88", footer }: {
  ...
  return (
    <div className="flex flex-col items-center justify-start h-full px-10 sm:px-20 pt-6"
      onClick={() => { advance(); }}
      role="button"
      tabIndex={-1}
      ...
      <div className="w-full max-w-5xl space-y-4 flex-1">
        <AnimatePresence initial={false}>
          {layers.slice(0, shown).map((layer, i) => (
            <motion.div key={i}
              ...
              <div className="px-5 py-2 text-xs uppercase tracking-[0.3em] mcb-mono"
                style={{ background: `${accent}18`, color: accent }}>{layer.label}</div>
              <div className="px-6 py-5 mcb-body text-gray-100">{layer.body}</div>
            </motion.div>
          ))}
        </AnimatePresence>
      </div>

      <div className="mt-6 mb-2 h-7 flex items-center gap-3 text-sm mcb-mono text-gray-400 select-none">
        {remaining > 0 ? (
          <>
      ...
      {footer && shown >= layers.length && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-2 mcb-meta text-gray-400 text-center max-w-3xl">{footer}</motion.div>
      )}
    </div>
  );
}
...
function CountdownTimer({ seconds = 300, ctx }: { seconds?: number; ctx: SlideCtx }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full px-10 text-center relative overflow-hidden">
      {expired && (
      ...
      <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 120 }}
        className="text-7xl sm:text-8xl mb-6">{expired ? "💀" : "🔒"}</motion.div>
      <motion.h2 initial={{ opacity: 0 }} animate={{ opacity: 1 }}
        className="mcb-h2 font-black mb-4"
      ...
      <p className="mcb-lead text-gray-300 max-w-3xl mb-10">
        {expired
      ...
      <motion.div className="mcb-mono font-black tracking-wider"
        ...
        style={{
          ...
          color: expired ? "#ef4444" : "#ef4444",
          textShadow: "0 0 30px rgba(239,68,68,0.6), 0 0 80px rgba(239,68,68,0.3)",
          lineHeight: 1,
        }}>
        {timeStr}
      </motion.div>
      <p className="mt-8 mcb-meta text-gray-500 max-w-3xl">
        {expired
...
function LiveExperiment({ src, label, hint }: { src: string; label: string; hint?: string }) {
  ...
  return (
    <div className="flex flex-col h-full px-3 pt-2 pb-1">
      <div className="flex items-center justify-between mb-2 px-2">
        <div className="flex items-center gap-3">
          <motion.span animate={{ opacity: [0.5, 1, 0.5] }} transition={{ repeat: Infinity, duration: 1.3 }}
            className="w-2.5 h-2.5 rounded-full bg-rose-500" style={{ boxShadow: "0 0 10px #f43f5e" }} />
          <span className="text-xs uppercase tracking-[0.3em] mcb-mono text-rose-300">CANLI DENEY</span>
          <span className="text-xs mcb-mono text-emerald-400/80">{label}</span>
        </div>
        <button onClick={() => setKey((k) => k + 1)}
          className="text-xs mcb-mono px-3 py-1.5 rounded-md border border-white/15 text-white/70 hover:text-white hover:border-white/40 bg-black/40 transition-colors cursor-pointer">
          ↻ Yeniden Başlat
        </button>
      </div>
      <div className="flex-1 rounded-xl overflow-hidden border border-emerald-500/20 mcb-glow-green bg-black">
        <iframe key={key} src={src} title={label}
          className="w-full h-full block"
          sandbox="allow-scripts allow-forms allow-same-origin allow-popups" />
      </div>
      {hint && <p className="mt-1 text-center mcb-meta text-gray-500">{hint}</p>}
    </div>
  );
}
...
const slides: Slide[] = [
  ...
  { id: "cover", content: (
    <div className="flex flex-col items-center justify-center h-full text-center px-8 relative">
      <motion.div className="absolute w-[640px] h-[640px] rounded-full border border-emerald-500/10"
        animate={{ scale: [0.85, 1.1, 0.85], opacity: [0.2, 0.5, 0.2] }} transition={{ repeat: Infinity, duration: 5 }} />
      <motion.div className="absolute w-[420px] h-[420px] rounded-full border border-cyan-500/10"
      ...
      <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 100 }}
        className="text-7xl mb-6 relative z-10" style={{ filter: "drop-shadow(0 0 18px rgba(0,255,136,0.5))" }}>🛡️</motion.span>
      <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25, duration: 0.7 }}
        className="mcb-h1 font-black mb-4 relative z-10"
      ...
      <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }}
        className="mcb-lead text-gray-300 max-w-3xl relative z-10">
  ...
  { id: "thanks", content: (
    <div className="flex flex-col items-center justify-center h-full text-center px-8">
      <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 110 }}
        className="text-7xl mb-6">🙏</motion.span>
      <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
        className="mcb-h1 font-black mb-6"
        style={{ color: "#00ff88", textShadow: "0 0 28px rgba(0,255,136,0.5)" }}>
        <GlitchText text="Teşekkürler" />
      </motion.h1>
      <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}
        className="mcb-lead text-gray-300 mb-10 max-w-3xl">
        Soru, paylaşım ve geri bildirim için:
      </motion.p>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8 }}
        className="mcb-body mcb-mono text-emerald-400">
        osmancancetlenbik.com
      </motion.div>
      <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.1 }}
        className="mt-3 mcb-meta text-gray-500">
...
export default function Presentation() {
  ...
  return (
    <div className="mcb-deck h-screen w-screen flex items-center justify-center bg-black" suppressHydrationWarning
      ...
      <div className="relative w-full h-full max-w-[177.78vh] max-h-[56.25vw] overflow-hidden select-none"
        ...
        <div className="absolute top-0 left-0 right-0 z-50 flex items-center justify-between px-6 py-3">
          <span className="text-sm mcb-mono text-emerald-400/70 uppercase tracking-wider"
            style={{ textShadow: "0 0 8px rgba(0,255,136,0.3)" }}>{sectionName}</span>
          <span className="text-sm mcb-mono text-emerald-400/40">{current + 1} / {total}</span>
        </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 z-50 bg-black/70 backdrop-blur-xl border-t border-emerald-500/10">
          <div className="flex items-center justify-between px-6 py-2.5">
            <span className="text-xs mcb-mono text-emerald-400/60 uppercase tracking-wider min-w-[160px]"
              style={{ textShadow: "0 0 6px rgba(0,255,136,0.3)" }}>{sectionName}</span>
            <div className="flex items-center gap-4">
              <button onClick={goPrev} disabled={current === 0}
                className="text-gray-500 hover:text-white text-xl border-none bg-transparent cursor-pointer disabled:opacity-20 transition-colors">‹</button>
            ...
            <span className="text-xs mcb-mono text-gray-500 min-w-[160px] text-right">{current + 1} / {total}</span>
          </div>
        </div>
      </div>
    </div>
